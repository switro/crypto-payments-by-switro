...
/**
 * Plugin Name: Crypto Payments By Switro for WC
 * Description: Accept solana and ETH payments instantly on your WooCommerce store with Switro – a modern, non-custodial crypto payment gateway.
 * Author: Switro
 * Author URI: https://switro.com/
 * Text Domain: crypto-payments-by-switro
...
// Copy to Clipboard script for webhook URL field
add_action('admin_enqueue_scripts', function () {
    $screen = get_current_screen();

    // Only load on WooCommerce admin screens
    if (! $screen || strpos($screen->id, 'woocommerce') === false) {
        return;
    }

    wp_enqueue_script(
        'switro-admin-webhook-copy',
        plugin_dir_url(__FILE__) . 'assets/js/admin-webhook-copy.js',
        array(),
        '1.0.0',
        true
    );
