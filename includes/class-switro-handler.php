...
class WC_Gateway_Switro_Handler
{
    ...
    public function register_webhook()
    {
        register_rest_route('switro/v1', '/webhook', [
            ...
            // This webhook must remain publicly accessible for Switro callbacks. 
            // Security checks and validation are handled within handle_webhook().
        ]);
    }
    ...
    public function handle_webhook(WP_REST_Request $request)
    {
        ...
        // This webhook endpoint is public and does not require authorization;

        // Log the raw JSON payload for debugging
        // error_log('[Switro Webhook] Payload: ' . print_r($body, true));

        // Switro sends the required identifiers (checkout_id, payment_id, etc.) in the request body.
        ...
        // If order is not found return with error
        if (empty($orders)) {
        ...
        if ($payment['status'] === 'confirmed') {
            ...
            $order->update_status('failed', __('Switro payment failed.', 'crypto-payments-by-switro'));
